...
def ms_update(request, id):
    ...
    if request.method == 'POST':
        ...
        revision_frqcy = str(request.POST.get('revision_frqcy'))
        ...
        updated_doc = Document(
            ...
            revision_frq= Frequency(revision_frqcy),
...
def ms_Doc_create(request):
    ...
    if request.method == 'POST' :
        ...
        clause_name = ISO_Clauses.objects.all(request.POST["clause_name"])
        document_type =  Doc_type.objects.all(request.POST["document_type"])
        purpose = request.POST['purpose']
        scope = request.POST['scope']
        document_owner = User.objects.get(username = request.POST['document_owner'])
        revision_frqcy = request.POST.get('revision_frqcy')
    ...
    return render(request, "MS/template/create_Doc.html", context= {"user_infor": user_infor, "clause_list": clause, "doc_type" : doc_type, "revision_frqcy":revision_frqcy})
